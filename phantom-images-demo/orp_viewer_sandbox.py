...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("❌ Nombre d'arguments incorrect. Usage: python orp_viewer_websocket_transition.py fichier.orp")
    ...
    if analysis['format_type'] == 'BINARY_PHANTOM':
        ...
        urn_id = analysis['data'].get('urn_id', 'unknown')
        access_data = analysis['data'].get('access_data', {})
        server_url = access_data.get('server_url', 'http://127.0.0.1:9300')
        ...
        if analysis['has_urn'] and analysis.get('requires_server', False):
            ...
            if validation_result["status"] not in ["validated", "no_urn"]:
                ...
                # Récupération de la matrice via HTTP après validation
                try:
                    print(f"🌐 Téléchargement matrice via {server_url}/phantom_matrix/{urn_id}")
                    matrix_response = requests.get(f"{server_url}/phantom_matrix/{urn_id}", timeout=120)
                    if matrix_response.status_code == 200:
                        matrix_json = matrix_response.json()
                        shape = tuple(matrix_json["shape"])
                        dtype = matrix_json["dtype"]
                        flat_data = matrix_json["data"]
                        image_data = np.array(flat_data, dtype=dtype).reshape(shape)
                        print(f"✅ Matrice reçue et reconstituée: {image_data.shape}")
                    else:
                        print(f"❌ Erreur HTTP matrice: {matrix_response.status_code}")
                except Exception as e:
                    print(f"❌ Exception téléchargement matrice: {e}")

        # Extraction données image JSON (fallback)
        if image_data is None and 'image_data' in analysis['data']:
